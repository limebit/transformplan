...
def main():
    # Fetch wine quality dataset from UCI ML Repository
    ...
    plan = (
        ...
        .map_discretize(
            ...
            new_column="quality_tier",
            right=True,
        )
        .math_divide_columns(
            column_a="fixed_acidity",
            column_b="volatile_acidity",
            new_column="acidity_ratio",
        )
        ...
        .math_percent_of(
            ...
            total_column="total_sulfur_dioxide",
            new_column="free_sulfur_pct",
            multiply_by=100.0,
        )
        .math_round(column="free_sulfur_pct", decimals=1)
        .math_rank(
            column="alcohol",
            new_column="alcohol_rank",
            method="ordinal",
            descending=True,
    ...
    # Do a dry-run to see if we can expect this to work
    preview = plan.dry_run(df)
    preview.print(show_params=False)
